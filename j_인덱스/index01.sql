...
# Key_name: PRIMARY (기본키 설정으로 자동 생성된 인덱스를 명시)
...
SELECT * FROM members; -- mem_id를 기준으로 오름차순 (ASP > BLK > IVE > LSP)
...
select * from members; -- 삽입순 출력
SHOW INDEX FROM members;

alter table members
add constraint unique(mem_id);

select * from members; -- 여전히 삽입순 출력
show index from members;
