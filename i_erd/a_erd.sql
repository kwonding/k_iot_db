...
# 1:N(일대다) - 한 엔티티가 여러 개와 관계 (교수 - 여러 개 강의, 학생 - 여러 개 수강 내역, 게시글 - 여러 개 댓글)
# N:N(다대다) - 여러 엔티티가 서로 다수와 관계 (여러 학생 - 여러 과목: 중간 테이블 필요)
...
# 수강 내역 테이블) 자식 테이블 - 주문 ID PK, 학번 FK, 강의명 ... 등
# >> 관계: 1명의 학생은 여러 개의 수강 내역을 가질 수 있음 (1:N 관계 - 실선)

# 3) 예시 2
# 주문과 주문 상세 내역
# 주문 테이블) 부모 테이블 - 주문ID PK, 주문 일시 ... 등
# 주문 상세 내역 테이블) 자식 테이블 - 주문ID PK, 주문 상품 ... 등
# >> 관계: 자식 테이블의 기본키가 부모의 기본키를 포함하거나 그대로 사용 (1:1 관계 - 실선)

# 4) 예시 3
# 회원과 즐겨찾기 프로그램
# 회원 테이블) 부모 - 회원ID PK, 이름 ... 등
# 즐겨찾기 프로그램 테이블) 고유번호ID PK, 회원 번호 FK, 프로그램명 ... 등
...
#		cf) 복합키: 두 개 이상의 컬럼으로 구성된 기본키(중복x, 비워둘수x)
