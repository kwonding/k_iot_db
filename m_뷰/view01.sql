...
# - 데이터 저장 X
# - 필요한 데이터만 보는 용도
# - 기본 테이블(참조하고있는 테이블) 변경 시 자동 반영
# >> CREATE VIEW 뷰명 AS SELECT문(TABLE명)
...
# >> 뷰는 테이블처럼 보이지만, 내부적으로는 SELECT문으로 작동됨
...
# cf) DB 개체들은 생성과 삭제 시 DDL 문법 사용 (CREATE, DROP)
DROP VIEW v_member_secure;

SELECT * FROM v_member_secure; # 오류남 drop했으니까
