...
SELECT distinct `Member Id`, `Member Name` FROM v_view_test01; # 중복 제거
...
# cf) 뷰 정보 확인
# : describe (desc) - 열(컬럼) 정보 확인 (PK, 제약조건은 확인 불가! / 뷰는 데이터만 가져옴)
DESCRIBE v_view_test02; -- key, default, extra 안보임
desc v_view_test02;

# cf) show create view 뷰이름;
# : 생성된 뷰의 SQL쿼리 확인
SHOW CREATE VIEW v_view_test02;
#VIEW `v_view_test02` AS select `member`.`mem_id` AS `mem_id`,`member`.`mem_name` AS `mem_name`
...
# > member 테이블로 생성한 v_member 뷰는 mem_id, addr만 존재한다고 가정
