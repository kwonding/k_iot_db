...
/*
    ...
    cf) savepoint: 트랜잭션 내 특정 지점 저장, 해당 지점으로 롤백(돌아가기) 가능 ROLLBACK
		savepoint 이름
			>> 되돌릴 수 있는 지점 저장
...
#		- 각 INSERT, UPDATE, DELETE 쿼리 실행 직후 자동 COMMIT됨
#		- 실패가 없으면 자동 저장됨
# > 0: 자동 커밋 비활성화 된 상태
#		- 트랜잭션은 START TRANSACTION 또는 DML 문으로 시작
#		- COMMIT 또는 ROLLBACK을 직접 호출 해야함
...
values
	(1, 2, '노트북', 200); -- 외래키 제약 조건 오류 (존재하지 않는 회원의 구매)
...
# if 조건문을 벗어나면 실행문(이체된 내역) ROLLBACK(취소); 
